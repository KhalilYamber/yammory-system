...
/** 七面多边形固定板块（用户画像条目 facet 归属）。 */
export type MemoryFacet = '躯体' | '心智' | '价值与意愿' | '能力与技能' | '行为与习惯' | '社会与处境' | '经历与轨迹'

/** 知识水平四档（level 1–10 的分档锚点）。 */
export type MemoryKnowledgeTier = '科普' | '本科' | '硕士' | '专家'

/** 条目状态（soft delete 预留；S5 前写入恒为 active）。 */
export type MemoryEntryStatus = 'active' | 'superseded'
...
export interface MemoryEntry {
  ...
  version: number
  /** 七面多边形板块归属（user 画像条目；agent 轨/无面为 null）。 */
  facet: MemoryFacet | null
  /** 分领域知识水平（1–10）；非知识类条目为 null。 */
  level: number | null
  /** 条目状态（soft delete 预留；S5 前恒为 active）。 */
