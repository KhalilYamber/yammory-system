 ...
 * 认领条件：toolName === 'memory' 且 reason 带 [yammory_system] 前缀——防止误伤
 * 其它插件以同名工具身份发出的审批请求。
 ...
 * action 词汇含连字符（profile-set 等），故首段用 `[a-z][a-z-]*` 而非纯字母。
...
export function parseWriteReason(reason) {
  const match = /^\[yammory_system\] ([a-z][a-z-]*)(?: \((\d+) entries\))? ([a-z]+)\/([a-z-]+)(?: \[source:([^\]]+)\])?\n([\s\S]*)$/.exec(reason)
