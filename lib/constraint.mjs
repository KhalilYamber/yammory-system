...
export function assertTierRules() {
  ...
  if (declared.length !== KNOWLEDGE_TIERS.length || declared.some((tier, index) => tier !== KNOWLEDGE_TIERS[index])) {
    throw new Error(`dsh-memento constraint: TIER_RULES tiers [${declared.join('|')}] must match KNOWLEDGE_TIERS [${KNOWLEDGE_TIERS.join('|')}] in the same order`)
  }
  for (const rule of TIER_RULES) {
    if (rule.zh === '' || rule.en === '') {
      throw new Error(`dsh-memento constraint: tier ${rule.tier} is missing its speaking requirement`)
    }
    for (let level = 1; level <= 10; level += 1) {
      const inside = level >= rule.minLevel && level <= rule.maxLevel
      if (inside !== (tierForLevel(level) === rule.tier)) {
        throw new Error(`dsh-memento constraint: tier ${rule.tier} range ${rule.minLevel}-${rule.maxLevel} disagrees with tierForLevel(${level}) = ${tierForLevel(level)}`)
      }
    }
  }
  if (!KNOWLEDGE_TIERS.includes(DEFAULT_TIER)) {
    throw new Error(`dsh-memento constraint: DEFAULT_TIER ${DEFAULT_TIER} is not one of ${KNOWLEDGE_TIERS.join('|')}`)
  }
}
