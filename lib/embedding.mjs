 ...
 * @property {boolean} [semantic] - 是否做**真实语义建模**。缺省视为 `true`（第三方注册即当语义）；
 *   伪嵌入/哈希类实现**必须显式声明 `false`**——`retrieval.vector` 的可用性门槛只认这个字段。
 ...
 *
 * **`semantic: false` 是声明，不是谦辞**：它按「连续字母/数字段」切词，中文整句即为一段，
 * 故「偏好」查不到「用户偏好：回复用中文」——拿它做语义召回等于静默关掉中文召回。
 * `retrieval.vector` 的选路因此不会选它。
 */
...
export class EmbeddingProviderRegistry {
  ...
  /**
   * 取第一个**声明为语义**的 provider（`semantic !== false`，按 id 升序取稳定首名）；没有则 undefined。
   * `retrieval.vector` 的选路走这里——伪嵌入永远不会被选中。
   * @returns {EmbeddingProvider | undefined} 语义 provider 或 undefined。
   */
  firstSemantic() {
    return [...this.providers.values()]
      .filter((provider) => provider.semantic !== false)
  ...
  #validate(/** @type {EmbeddingProvider} */ provider) {
    ...
    if (provider.semantic !== undefined && typeof provider.semantic !== 'boolean') {
      throw new InvalidInputError('embedding provider semantic flag must be a boolean when present')
    }
  }
}
