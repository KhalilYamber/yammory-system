...
/**
 * 自动整理的来源标注（F8 把握分级）：落写审计的 source 列。有了它，`source:tidy-auto`
 * 就是一个独立粒度键——自动这条路能单独设 writePolicy（ask/auto/off），与模型手动整理的
 * 默认来源互不牵连；关掉它时自动整理静默不动，手动路径照常。
 */
export const AUTO_TIDY_SOURCE = 'tidy-auto'
...
/** 条目 id 形状：UUID v4（Provider 生成，跨会话稳定）。与 MAX_TAGS 同法：从常量表取用并再导出。 */
export { ENTRY_ID_PATTERN }
 ...
 * @property {(input: object) => {superseded: ProtocolEntry[], entry: ProtocolEntry | null, cleared: {id: string, createdAt: number, status: string} | null}} supersedeEntries
 * @property {(input: {batchId: string, producedIds: string[], sourceIds: string[], audit?: object}) => {restored: ProtocolEntry[], demoted: ProtocolEntry[]}} rollbackBatch
 ...
 * @property {(batchId: string) => ProtocolEntry[]} entriesByBatch
 ...
 * @param {Record<string, string>} [deps.writePolicies] - 粒度写策略表（`source:<名字>` / `track/scope`）：只用于审计标注生效策略。
  ...
  constructor(deps) {
    ...
    // 粒度写策略表（可空）：只用来**如实标注**审计 outcome——哪一条策略放行/拒绝了这次写。
    // 少了它，拒绝行的文案只能拿全局策略充数，把 off 写成 ask，账目分不清拒因。
    this.writePolicies = deps.writePolicies ?? {}
  ...
  async seed(inputs, write) {
    ...
    this.store.auditAppend({
      ...
      text: summary, outcome: this.#outcomeLabel(via, { track: 'batch', scope: 'batch', ...(gateSource === undefined ? {} : { source: gateSource }) }),
  ...
  async consolidate(input, write) {
    ...
    for (const old of removed) {
      this.store.auditAppend({
        ...
        outcome: this.#outcomeLabel(via, { track, scope, source: old.source }), source: old.source, sessionId,
      })
    ...
    this.store.auditAppend({
      ...
      outcome: this.#outcomeLabel(via, { track, scope, source: entry.source }), source: entry.source, sessionId,
    })
    this.#appendWriteEvent(write, SESSION_EVENTS.added, { entry, source: entry.source })
  ...
  async requestTidy(input, write) {
    ...
    this.store.auditAppend({
      ...
      text: null,
      outcome: `${created ? 'registered' : 'already-pending'} ${this.#outcomeLabel(via, { track: LIBRARY_BUCKET.track, scope: LIBRARY_BUCKET.scope, source })}`,
      source,
   ...
   * @param {import('../types.js').MemoryWriteContext} write - {agent, callId?, signal?, gate?}。
   * @param {{batchId?: string | null}} [internal] - 核心内部参数：只有自动整理路径传批次号（工具面与命令面不经过这里），故批次号不可由调用方伪造。
   ...
   */
  async supersede(input, write, internal = {}) {
    this.#assertAgent(write)
    ...
    // 批次号只由核心内部的自动整理路径给出（第三个参数不经工具面/命令面），因此不可伪造：
    // 它贯穿产出条目、降级条目与全部审计行，是「按批撤回」与「按批重建」的唯一锚点。
    const batchId = typeof internal.batchId === 'string' && internal.batchId.length > 0 ? internal.batchId : null
    ...
    this.#throwIfAborted(write)
    // 写方会话 id：显式收窄成 string | null（checkJs 下 session?.id 会落成 unknown）。
    const sessionId = /** @type {string | null} */ (write.agent.session?.id ?? null)
    const outcome = this.#outcomeLabel(via, { track, scope, source })
    // 产出条目的 id 在这里先铸出来，随条目一起交给事务。UUID 是本地现铸的，落库前就已知，
    // 「早生」不违反任何不变量——反过来，等插入时再生，摘要行就只能留在事务外，于是整类
    // 「账与库撕开」的问题都从这一处冒出来（红队四轮 N1–N4）。放在这里，整批账目与条目
    // 同一个提交：写不成就是全不成，没有需要事后补偿的半成品。
    const entryId = mergeText === undefined ? null : randomUUID()
    // 待整理标记的 id 也在写之前读出，随事务一起收边：标记是队列里的一条权威状态，
    // 留在事务外就等于「整批成了、标记没结」，下一轮重复消费一次（红队组级复核指出）。
    const pendingMarker = /** @type {{id: string} | null} */ (/** @type {unknown} */ (this.store.tidyRequestPending()))
    const { superseded, entry, cleared } = this.store.supersedeEntries({
      ids,
      ...(mergeText === undefined ? {} : { text: mergeText, tags, id: /** @type {string} */ (entryId) }),
      facet,
      level,
      source,
      workspaceKey,
      ...
      audit: {
        action: 'supersede',
        base: { outcome, source, sessionId, batchId },
        addText: MERGED_TAG,
        clearTidyRequestId: pendingMarker === null ? null : pendingMarker.id,
      },
    })
    if (cleared !== null) {
      try {
        ...
        // 收边账目缺失不改变「整批已落地」的事实：批次面看的是条目与批次行。
      }
    }
    // 会话事件（改写史的播报）：与撤回路径同档，整批已成再派发，不早于提交、不晚于返回。
    // 事件只是播报，不进权威状态：派发失败也不回退（回退会与已落库的批次相抵）。
   ...
   * @param {import('../types.js').MemoryWriteContext} write - {agent, callId?, signal?, gate?}。
   * @returns {Promise<{grade: ReturnType<typeof gradeMerge>, batchId: string, superseded: ProtocolEntry[], entry: ProtocolEntry | null, usage: {track: string, scope: string, used: number, limit: number}, tidyRequestCleared: boolean}>} 分级依据（含逐杠明细，可复述）、本批批次号与落写结果。
   */
  async autoTidy(input, write) {
    ...
    // 来源由动作自己钉死，不接受调用方覆盖：`source:tidy-auto` 这个粒度键要可信，
    // 否则「把自动整理设成 off」会被一个伪造的来源名绕过，审计也分不出这活是谁干的。
    // 每一轮自动整理 = 一个批次：批次号在此铸出，贯穿产出条目、降级条目与全部审计行。
    ...
    return { grade, batchId, ...result }
  }

  /**
   * 批次留痕的只读重建（F8 批次面）：仅凭审计行还原一整批——源 id 清单、产出条目 id、
   * 来源、会话、起止时间戳，外加该批次涉及的两侧条目（产出条目与降级条目）。
   * 纯只读：不落审计、不走审批门、不改任何行；未知批次返回 null（「查无此批」不是异常）。
   * @param {string} batchId - 批次号（非空字符串；形状非法由 Provider 响亮拒绝）。
   * @returns {{batchId: string, rows: object[], sourceIds: string[], producedIds: string[], source: string | null, sessionId: string | null, startedAt: number, endedAt: number, rolledBack: boolean, entries: ProtocolEntry[]} | null} 批次报告；未知批次为 null。
   */
  batchReport(batchId) {
    const rows = /** @type {Array<{action: string, entryId: string | null, source: string | null, sessionId: string | null, ts: number}>} */ (this.store.auditByBatch(batchId))
    if (rows.length === 0) return null
    /** @param {string} action - 审计动作名。 @returns {string[]} */
    const idsOf = (action) => rows.filter((row) => row.action === action).map((row) => row.entryId).filter((id) => typeof id === 'string')
    return {
      batchId,
      rows,
      // 降级行只记 id（text 恒 null），产出条目另有 supersede-add 行——两者即「合了哪些、产出哪一条」。
      sourceIds: idsOf('supersede'),
      producedIds: idsOf('supersede-add'),
      // 来源取「本批动作自己的来源」，不是被降级条目的历史出处：降级行的 source 记的是
      // old.source（条目原来的出处），批量来源落在摘要行与产出行上（自动整理恒为 tidy-auto）。
      source: rows.find((row) => row.action === 'consolidation' && typeof row.source === 'string' && row.source.length > 0)?.source
        ?? rows.find((row) => row.action === 'supersede-add' && typeof row.source === 'string' && row.source.length > 0)?.source
        ?? null,
      sessionId: rows.find((row) => typeof row.sessionId === 'string' && row.sessionId.length > 0)?.sessionId ?? null,
      startedAt: rows[0].ts,
      endedAt: rows[rows.length - 1].ts,
      rolledBack: rows.some((row) => row.action === 'restore-batch'),
      entries: /** @type {ProtocolEntry[]} */ (this.store.entriesByBatch(batchId)),
    }
  }

  /**
   * 整批撤回（F8 批次面）：把一批自动整理整体回滚——产出的合并条目降级、被它降级的源条目
   * 一次性恢复为 `active`，两件事在 Provider 的**同一个事务**里完成（要么全成、要么全不动）。
   * 复用既有 restore 的单向语义：只允许 `superseded → active`；产物必须是 active，否则视为
   * 「这批已经撤回过了」响亮失败。只碰带该批次号的条目：非本批次条目零影响。
   * @param {{batchId: string, source?: string}} input - {batchId, source?}（source 缺省走治理面标注 governance）。
   * @param {import('../types.js').MemoryWriteContext} write - {agent, callId?, signal?, gate?}。
   * @returns {Promise<{batchId: string, restored: ProtocolEntry[], demoted: ProtocolEntry[], usage: {track: string, scope: string, used: number, limit: number}}>} 恢复的源条目、降级的产出条目与用量。
   */
  async restoreBatch(input, write) {
    this.#assertAgent(write)
    this.#assertSessionOn('restore-batch', write)
    if (input === null || typeof input !== 'object') {
      throw new InvalidInputError('restore-batch input must be an object')
    }
    // 只读面先取批次账：未知批次、形状非法都在打扰用户之前响亮失败（Provider 的入参防御同样生效）。
    const report = this.batchReport(input.batchId)
    if (report === null) {
      // 审计行被 retentionDays 裁掉时，条目还在、批次却「查无此批」——如实说清，别让人以为批次不存在。
      const orphans = /** @type {ProtocolEntry[]} */ (this.store.entriesByBatch(input.batchId))
      if (orphans.length > 0) {
        throw new InvalidInputError(`batch ${JSON.stringify(input.batchId)} still has ${orphans.length} entries but its audit rows are gone (pruned by auditRetentionDays); the batch can no longer be rebuilt as a whole — restore those entries one by one with /memory restore <id>`)
      }
      throw new InvalidInputError(`no batch ${JSON.stringify(input.batchId)} in the audit ledger; nothing was changed`)
    }
    const first = report.entries[0]
    if (first === undefined) {
      throw new InvalidInputError(`batch ${JSON.stringify(report.batchId)} has no entries to roll back; nothing was changed`)
    }
    // 已撤回过的批次在**打扰用户之前**就拒绝：先弹审批再报「已经撤回过了」，既白耗一次许可，
    // 载荷的方向还是反的（把已恢复的源写成降级、把产物写成恢复），命令面读起来像要再撤一次。
    if (report.rolledBack) {
      throw new InvalidInputError(`batch ${JSON.stringify(report.batchId)} is already rolled back; nothing was changed`)
    }
    // 产出条目必须仍在场（规格原文：「产出必须是 active，否则视为已撤回并响亮失败」）。
    // 只看 rolledBack 不够：产物被手工降级、而账本里没有撤回行时，撤回会成功返回
    // demoted 1 的「假动作」——报了一条没发生的降级（红队裁决 G1）。这里按状态如实拒绝。
    const producedAway = report.producedIds.filter((id) => this.store.entryById(id)?.status !== ACTIVE_STATUS)
    if (producedAway.length > 0) {
      throw new InvalidInputError(`batch ${JSON.stringify(report.batchId)} has already been rolled back (produced entry ${producedAway.join(', ')} is no longer active); nothing was changed`)
    }
    // 账目不齐的批次（产出行被裁掉/手工删掉）在这里就拦住，别等到 Provider 的入参校验里
    // 报「product ids 形状非法」——那是参数错误的口吻，用户读不出「账不全、请逐 id 恢复」这条退路。
    if (report.entries.length > 0 && report.producedIds.length === 0) {
      throw new InvalidInputError(`batch ${JSON.stringify(report.batchId)} has entries but no produced-entry audit row (pruned or removed); the batch can no longer be rebuilt as a whole — recover the demoted source entries one by one with /memory restore <id>`)
    }
    // 会话可见集（与 restore 同档）：本会话看不见的批次不许由本会话撤回——批次面不是越权口子。
    const visibleAgentKey = this.#agentKeyOf(write)
    const visibleWorkspaceKey = this.#workspaceKeyOf(write)
    for (const entry of report.entries) this.#assertVisible(entry, visibleAgentKey, visibleWorkspaceKey)
    const { track, scope } = first
    const source = input.source ?? GOVERNANCE_SOURCE
    const plan = [
      `rollback batch: ${report.batchId}`,
      ...report.entries.filter((entry) => entry.status === ACTIVE_STATUS).map((entry) => `demote (batch product): ${entry.id}\n${snippetOf(entry.text)}`),
      ...report.entries.filter((entry) => entry.status === SUPERSEDED_STATUS).map((entry) => `restore (superseded by this batch): ${entry.id}\n${snippetOf(entry.text)}`),
    ].join('\n')
    const via = await this.#ask({ action: 'restore-batch', track, scope, text: plan, source }, write)
    this.#throwIfAborted(write)
    // 审批窗口复检（与 arbitrate 的 champion 复检同型）：审批期间条目被改写（version 变）或改了状态，
    // 落库的就会与人类在载荷里看到的那一份不符——宁可响亮失败让人重读，也不静默吞掉编辑。
    const fresh = this.batchReport(report.batchId)
    const approved = new Map(report.entries.map((entry) => [entry.id, entry]))
    const drifted = (fresh?.entries ?? []).filter((entry) => {
      const before = approved.get(entry.id)
      return before === undefined || before.version !== entry.version || before.status !== entry.status
    })
    if (fresh === null || fresh.entries.length !== report.entries.length || drifted.length > 0) {
      throw new InvalidInputError(`batch ${JSON.stringify(report.batchId)} changed while the rollback waited for approval (${drifted.map((entry) => entry.id).join(', ') || 'entries disappeared'}); nothing was changed — re-read the batch and roll back again`)
    }
    const sessionId = /** @type {string | null} */ (write.agent.session?.id ?? null)
    // 回退与账目同事务：条目状态与账本要么一起成、要么一起不成。回执留在事务外时，一次审计
    // 写失败就能把账与库撕开——库已撤回而 rolledBack 仍为 false，面板按钮永远点不动（N2）。
    const outcome = this.#outcomeLabel(via, { track, scope, source })
    const { restored, demoted } = /** @type {{restored: ProtocolEntry[], demoted: ProtocolEntry[]}} */ (this.store.rollbackBatch({
      batchId: report.batchId,
      producedIds: report.producedIds,
      sourceIds: report.sourceIds,
      audit: {
        track,
        scope,
        // 摘要按**事务内实际**动过的条目渲染：计划清单可能与实际不符（例如某条源条目
        // 已被手工救回，本次就不需要恢复它），按计划写会报出没发生的事。
        summary: (/** @type {{restored: ProtocolEntry[], demoted: ProtocolEntry[]}} */ { restored, demoted }) => `rolled back batch ${report.batchId}: restored ${restored.length} (${restored.map((entry) => entry.id).join(', ')}); demoted ${demoted.length} (${demoted.map((entry) => entry.id).join(', ')})`,
        demoteOutcome: outcome,
        restoreOutcome: outcome,
        base: { source, sessionId, batchId: report.batchId },
      },
    }))
    ...
    return { batchId: report.batchId, restored, demoted, usage: this.#usage(track, scope) }
  }
  ...
  async arbitrate(input, write) {
    ...
    this.store.auditAppend({
      ...
      outcome: this.#outcomeLabel(via, { track, scope, source }),
      source,
      sessionId,
    })
  ...
  async setProfile(input, write) {
    ...
    this.store.auditAppend({
      ...
      entryId: null,
      text: plan,
      outcome: this.#outcomeLabel(via, { track: 'user', scope: 'user-global', source }),
      source,
      sessionId: write.agent.session?.id ?? null,
    })
    ...
    try {
      ...
      if (error instanceof WriteDeniedError) {
        ...
        const viaLabel = typeof write.gate === 'function'
          ...
          : `${outcome} (via approval, writePolicy ${this.#effectivePolicy(payload)})`
        this.store.auditAppend({
  ...
  #outcomeLabel(/** @type {{outcome: string, source: 'approval'|'gate'}} */ via, /** @type {{track?: string, scope?: string, source?: string}} */ payload) {
    return via.source === 'gate'
      ? `${via.outcome} (via write gate)`
      : `${via.outcome} (via approval, writePolicy ${payload === undefined ? this.writePolicy : this.#effectivePolicy(payload)})`
  }

  /**
   * 本次写**生效**的写策略（与实际裁决同一套优先级：`source:<来源>` 精确键 > `track/scope` > 全局）。
   * 审计里记的是这个值，不是配置里的全局值——否则账目分不清「策略 off 拒的」与「ask 无人应答」。
   * @param {{track?: string, scope?: string, source?: string}} payload - 写载荷。
   * @returns {string} 生效策略。
   */
  #effectivePolicy(payload) {
    const key = typeof payload.source === 'string' && payload.source.length > 0 ? `source:${payload.source}` : undefined
    if (key !== undefined && typeof this.writePolicies[key] === 'string') return this.writePolicies[key]
    const bucketKey = typeof payload.track === 'string' && typeof payload.scope === 'string' ? `${payload.track}/${payload.scope}` : undefined
    if (bucketKey !== undefined && typeof this.writePolicies[bucketKey] === 'string') return this.writePolicies[bucketKey]
  ...
  /** 治理面审计行（S5）：restore 与 arbitrate 共用形状，`text` 由调用方决定（降级/打标行传 null）；批次面动作另带批次号。 */
  #auditGovernance(/** @type {string} */ action, /** @type {ProtocolEntry} */ entry, /** @type {import('../types.js').MemoryWriteContext} */ write, /** @type {{outcome: string, source: 'approval'|'gate'}} */ via, /** @type {string | null} */ sessionId, /** @type {string | null} */ text, /** @type {string | null} */ batchId = null) {
    this.store.auditAppend({
      action,
      track: entry.track,
      scope: entry.scope,
      entryId: entry.id,
      text,
      outcome: this.#outcomeLabel(via, { track: entry.track, scope: entry.scope, source: entry.source }),
      source: entry.source,
      sessionId,
      batchId,
    })
  ...
  #auditWrite(/** @type {string} */ action, /** @type {string} */ track, /** @type {string} */ scope, /** @type {ProtocolEntry} */ entry, /** @type {import('../types.js').MemoryWriteContext} */ write, /** @type {{outcome: string, source: 'approval'|'gate'}} */ via) {
    this.store.auditAppend({
      ...
      entryId: entry.id,
      text: entry.text,
      outcome: this.#outcomeLabel(via, { track, scope, source: entry.source }),
      source: entry.source,
      sessionId: write.agent.session?.id ?? null,
    })
  }
