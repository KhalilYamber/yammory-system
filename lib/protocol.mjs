 ...
 * @property {() => import('../types.js').MemoryEntry[]} allEntries
 ...
 * @property {(input: object) => {superseded: import('../types.js').MemoryEntry[], entry: import('../types.js').MemoryEntry | null}} supersedeEntries
 * @property {(id: string) => import('../types.js').MemoryEntry | null} entryById
...
export class MemoryProtocolCore {
   ...
   * @param {import('../types.js').MemoryWriteContext} write - {agent, callId?, signal?, gate?}。
   * @returns {Promise<{superseded: import('../types.js').MemoryEntry[], entry: import('../types.js').MemoryEntry | null, usage: {track: string, scope: string, used: number, limit: number}}>}。
   */
  async supersede(input, write) {
    ...
    const targets = ids.map((id) => {
      ...
      if (entry.status !== 'active') throw new InvalidInputError(`entry ${JSON.stringify(id)} is already ${entry.status}; only active entries can be superseded`)
    ...
    return { superseded, entry, usage: this.#usage(track, scope) }
  }
   ...
   */
  #assertVisible(/** @type {import('../types.js').MemoryEntry} */ entry, /** @type {string} */ agentKey, /** @type {string} */ workspaceKey) {
...
/**
 * 降级 id 列表校验（1..MAX_CONSOLIDATE_MATCHES 个非空字符串、互不重复）。
 * 重复 id 会在第二遍被当成「已降级」而误报，故在入口就响亮拒绝。
 ...
 * 混装会把 A 桶的正文搬进 B 桶。
 * @param {import('../types.js').MemoryEntry[]} targets - 已定位的降级目标（非空）。
 */
function assertSameBucket(targets) {
  const first = targets[0]
  const key = (/** @type {import('../types.js').MemoryEntry} */ entry) => [entry.track, entry.scope, entry.agentKey, entry.scope === 'workspace' ? entry.workspaceKey : ''].join('\u0000')
  const expected = key(first)
  for (const entry of targets) {
    if (key(entry) !== expected) {
      throw new InvalidInputError(
        `supersede must stay inside one bucket (track × scope × agentKey${first.scope === 'workspace' ? ' × workspaceKey' : ''}): ${JSON.stringify(first.track)}/${JSON.stringify(first.scope)}/${JSON.stringify(first.agentKey)} mixes with ${JSON.stringify(entry.track)}/${JSON.stringify(entry.scope)}/${JSON.stringify(entry.agentKey)}`,
      )
    }
  }
}
