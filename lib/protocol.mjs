 ...
 * @property {(input: {batchId: string, producedIds: string[], sourceIds: string[]}) => {restored: ProtocolEntry[], demoted: ProtocolEntry[]}} rollbackBatch
...
export class MemoryProtocolCore {
  ...
  async supersede(input, write, internal = {}) {
    ...
    for (const old of superseded) {
      ...
      this.store.auditAppend({
        ...
        outcome: this.#outcomeLabel(via), source: old.source, sessionId, batchId,
      })
      this.#appendWriteEvent(write, SESSION_EVENTS.removed, { entry: old, source: old.source })
    }
    if (entry !== null) {
      this.store.auditAppend({
        ...
        outcome: this.#outcomeLabel(via), source: entry.source, sessionId, batchId,
      })
      this.#appendWriteEvent(write, SESSION_EVENTS.added, { entry, source: entry.source })
    }
  ...
  batchReport(batchId) {
    ...
    return {
      ...
      // old.source（条目原来的出处），批量来源落在摘要行与产出行上（自动整理恒为 auto-tidy）。
  ...
  async restoreBatch(input, write) {
    ...
    const { restored, demoted } = /** @type {{restored: ProtocolEntry[], demoted: ProtocolEntry[]}} */ (this.store.rollbackBatch({ batchId: report.batchId, producedIds: report.producedIds, sourceIds: report.sourceIds }))
    for (const entry of demoted) {
      // 与整理机同形：降级行只记 id，text 恒为 null（降级是元数据动作，审计不复制正文）。
      // 动作名独立于 'supersede'：否则「本批合了哪些」的源 id 清单会把产出条目也算进去。
      this.#auditGovernance('restore-batch-demote', entry, write, via, sessionId, null, report.batchId)
      this.#appendWriteEvent(write, SESSION_EVENTS.removed, { entry, source: entry.source })
    }
    for (const entry of restored) {
      this.#auditGovernance('restore', entry, write, via, sessionId, String(entry.text), report.batchId)
      this.#appendWriteEvent(write, SESSION_EVENTS.added, { entry, source: entry.source })
    }
    // 收尾回执（与整理机的 consolidation 行同形）：一行摘要，`/memory audit` 可查、批次可重建。
    this.store.auditAppend({
      action: 'restore-batch',
      track,
      scope,
      entryId: null,
      text: `rolled back batch ${report.batchId}: restored ${restored.length} (${restored.map((entry) => entry.id).join(', ')}); demoted ${demoted.length} (${demoted.map((entry) => entry.id).join(', ')})`,
      outcome: this.#outcomeLabel(via),
      source,
      sessionId,
