...
export class MemoryProtocolCore {
   ...
   * @param {import('../types.js').MemoryWriteContext} write - {agent, callId?, signal?, gate?}。
   * @returns {Promise<{superseded: ProtocolEntry[], entry: ProtocolEntry | null, usage: {track: string, scope: string, used: number, limit: number}}>}。
   */
   ...
   */
  #resolveByIds(ids, write, expectedStatus = ACTIVE_STATUS) {
    ...
    return ids.map((id) => {
      ...
      if (entry.status !== expectedStatus) {
        const hint = expectedStatus === SUPERSEDED_STATUS
          ...
          : 'only active entries can be arbitrated (use restore to bring a superseded entry back first)'
...
function assertSameBucket(targets) {
  ...
  const key = (/** @type {ProtocolEntry} */ entry) => [entry.track, entry.scope, entry.agentKey, entry.scope === 'workspace' ? entry.workspaceKey : ''].join('\u0000')
