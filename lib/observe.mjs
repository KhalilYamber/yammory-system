...
/**
 * 从一个会话日志快照的事件流里抽出真人发言（闸二＋单条截断）。
 * @param {unknown} events - 事件数组（形状不信任，防御性解析）。
 * @param {number} maxChars - 单条发言字符上限（超出即截断加省略号）。
 * @returns {{messages: Array<{seq: number, at: number, text: string, truncated: boolean}>, injected: number}} 发言清单与被闸二挡下的伪发言条数。
 */
export function extractHumanMessages(events, maxChars) {
  ...
  for (const event of events) {
    ...
    messages.push({
      ...
      text: over ? `${text.slice(0, maxChars - 1)}…` : text,
      truncated: over,
    })
  }
 ...
 * @property {{sessions: number, read: number, messages: number, injected: number}} scanned - 窗口内候选会话数、真正读过的会话数、读到的真人发言数、被闸二挡下的伪发言数。
 ...
 */
export function buildObservationSlice(sessions, options, now, labels) {
  ...
  return {
    ...
    scanned: { sessions: candidates.length, read: selected.length, messages: scannedMessages, injected },
