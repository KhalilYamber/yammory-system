...
import { TRACKS, SCOPES, EXPERIENCE_TOPIC_RESERVED_TAGS, EXPERIENCE_DATE_TAG_PATTERN, WARMUP_TOPIC_CAP } from './constants.mjs'
 ...
 * @property {string[]} [tags] - 标签（经验目录取话题标用；缺省即无标）。
 */
...
/**
 * 取经验条目的话题：tags 里第一个非保留标（保留词表与日期标形状见 lib/constants.mjs）。
 * 没有则空串——话题只是目录线索，正文仍只在 memory_recall 里出现。
 * @param {SnapshotEntry} entry - 条目。
 * @returns {string} 话题名（无则空串）。
 */
export function topicOfEntry(entry) {
  const tags = Array.isArray(entry.tags) ? entry.tags : []
  for (const tag of tags) {
    if (typeof tag === 'string' && tag.length > 0 && !EXPERIENCE_TOPIC_RESERVED_TAGS.includes(tag) && !EXPERIENCE_DATE_TAG_PATTERN.test(tag)) return tag
  }
  return ''
}
...
export function renderWarmup(entries, profileRows = [], budgets = undefined, language = 'en', proposals = []) {
  ...
  // 经验目录（S4b-6+）：agent 轨的话题去重后按名升序，最多 WARMUP_TOPIC_CAP 个。
  // 只报话题名（取自 tags），正文一律不进这块冻结文本。
  const experienceEntries = entries.filter((entry) => entry.track === 'agent')
  const topics = [...new Set(experienceEntries.map(topicOfEntry).filter((topic) => topic.length > 0))].sort().slice(0, WARMUP_TOPIC_CAP)
  ...
  if (onDemand > 0) {
    sections.push(/** @type {(info: {onDemand: number, experienceCount: number, topics: string[]}) => string} */ (pick(WARMUP_DIRECTORY, language))({ onDemand, experienceCount: experienceEntries.length, topics }))
  }
