...
import {
  ...
  WARMUP_HEADER, WARMUP_PROFILE_TITLE, pick,
} from './strings.mjs'
...
export function renderWarmup(entries, profileRows = [], budgets = undefined, language = 'en', proposals = []) {
  const resident = residentEntries(entries)
  if (resident.length === 0 && profileRows.length === 0 && proposals.length === 0) return ''
  ...
  const sections = [renderConstraint(profileRows, language)]
  if (resident.length > 0) {
    ...
    const sorted = [...resident].sort((a, b) => a.createdAt - b.createdAt || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))
    sections.push(`## ${profileTitle} — ${row.used}/${row.limit} ${usageSuffix}\n${sorted.map((entry) => `- ${entry.text}`).join('\n')}`)
  }
