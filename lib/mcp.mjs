...
//
// 让外部 MCP 客户端（Claude / Codex 等）以只读方式消费 yammory_system 记忆库：
...
// Provider seam，默认取 SubstringRetriever（大小写不敏感子串）；MCP 面刻意不跟
// memory_recall 的 keyword 默认走——外部客户端看到的是显式、可预期的字面命中，
// 想换检索器可传 opts.retriever。
...
export const MCP_SERVER_NAME = 'yammory_system'
...
const MEMORY_SEARCH_TOOL = {
  name: 'memory_search',
  description: 'Search the yammory_system memory store by case-insensitive substring and return ranked entries (read-only).',
  inputSchema: {
    type: 'object',
    properties: {
