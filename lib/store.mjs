 ...
 * @property {(input: ProfileInput) => object} profileUpsert
 * @property {() => object[]} profileList
 * @property {(domain: string) => object | null} profileGet
 * @property {() => void} close
 */
...
export function openMemoryStore(dbPath, options = {}) {
  ...
  const store = {
    ...
    /** @returns {object[]} 全部画像行（按 domain 升序）。 */
    profileList() {
     ...
     * @returns {object | null} 画像行；未记录返回 null。
     */
    profileGet(domain) {
...
function insertOne(/** @type {Db} */ db, /** @type {StoreInsertInput} */ input) {
  ...
  const facet = /** @type {MemoryFacet | null} */ (input.facet ?? null)
  if (facet !== null && !PROFILE_FACETS.includes(facet)) {
...
/** v5 增量：七面多边形 facet + 分领域知识 level + soft delete 预留 status + profile 表。 */
const V5_SCHEMA_SQL = `
  ALTER TABLE entries ADD COLUMN facet TEXT;
  ALTER TABLE entries ADD COLUMN level INTEGER;
