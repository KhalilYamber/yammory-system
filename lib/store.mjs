...
import { SCHEMA_VERSION, DEFAULT_DB_NAME, TRACKS, SCOPES, PROFILE_FACETS, KNOWLEDGE_DOMAINS, KNOWLEDGE_TIERS, tierForLevel, ERROR_CODES, MAX_QUERY_LIMIT, MAX_CONSOLIDATE_MATCHES, MAX_TAGS_PER_ENTRY, SESSION_DEFAULT_ENABLED, ACTIVE_STATUS, SUPERSEDED_STATUS } from './constants.mjs'
...
export function openMemoryStore(dbPath, options = {}) {
  ...
  const store = {
    ...
    auditAppend(row) {
      const ts = Date.now()
      db.prepare(`INSERT INTO audit (ts, action, track, scope, entry_id, text, outcome, source, session_id, batch_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
...
function insertOne(/** @type {Db} */ db, /** @type {StoreInsertInput} */ input) {
  ...
  db.prepare(`INSERT INTO entries (id, track, scope, workspace_key, agent_key, text, source, tags, version, created_at, updated_at, session_id, facet, level, status, batch_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
