  ...
  const store = {
    ...
    replaceEntry(input) {
      ...
      return withTransaction(db, () => {
        ...
        db.prepare('UPDATE entries SET text = ?, tags = ?, facet = ?, level = ?, version = version + 1, updated_at = ?, session_id = ? WHERE id = ?')
          .run(input.text, JSON.stringify(tags), facet, level, Date.now(), input.sessionId ?? null, target.id)
