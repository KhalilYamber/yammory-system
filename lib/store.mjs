...
import { ENTRY_ID_PATTERN, SCHEMA_VERSION, DEFAULT_DB_NAME, TRACKS, SCOPES, PROFILE_FACETS, KNOWLEDGE_DOMAINS, KNOWLEDGE_TIERS, tierForLevel, ERROR_CODES, MAX_QUERY_LIMIT, MAX_CONSOLIDATE_MATCHES, MAX_TAGS_PER_ENTRY, SESSION_DEFAULT_ENABLED, ACTIVE_STATUS, SUPERSEDED_STATUS } from './constants.mjs'
import { findUniqueMatch, requireUniqueMatch } from './match.mjs'
// 桶键只有一个出处（lib/consolidate.mjs）：store 里再拼一份，将来列或分隔符一改，两处就会各说各话。
import { bucketKeyOf } from './consolidate.mjs'
 ...
 * @property {string} [id] - 条目 id（省略则现铸 UUID）。让调用方**提前**给出 id，是为了让「产出条目哪一条」在落库前就已知，账目才能与条目收进同一个事务。
 ...
 * @property {string | null} [sessionId]
 * @property {string | null} [batchId] - 自动整理批次号（只由协议层的自动路径铸出；普通整理为 null）。
 * @property {string} [id] - 产出条目的 id（协议层提前铸出：id 落库前已知，摘要行才能与条目同事务）。省略则 Provider 现铸。
 * @property {{action: string, base: {outcome: string, source: string, sessionId: string | null, batchId: string | null}, addText: string, clearTidyRequestId?: string | null}} [audit] - 本批账目（同事务内按实际条目编行）：无此字段即不记账（store 直调形状）。
 * @typedef {object} StoreIdListInput - S5 治理：按 id 批量动状态/标签（回滚、打标）。
 * @property {string[]} ids - 目标条目 id（1..MAX_CONSOLIDATE_MATCHES 个，须互不重复）。
 * @typedef {object} StoreRollbackInput - F8 批次面：按批次整批回滚（产出条目降级 ＋ 源条目恢复，单事务）。
 * @property {string} batchId - 批次号。
 * @property {string[]} producedIds - 该批次产出的条目 id（协议层从审计行还原后传入：Provider 不猜动作语义）。
 * @property {string[]} sourceIds - 该批次降级的源条目 id（同样由协议层从审计行还原）：按这份清单逐条对账，不做「查一遍当前批次号」。
 * @property {{track: string, scope: string, summary: (lines: {restored: MemoryEntry[], demoted: MemoryEntry[]}) => string, demoteOutcome: string, restoreOutcome: string, base: {source: string, sessionId: string | null, batchId: string | null}}} [audit] - 撤回账目（同事务插入）：降级行 + 恢复行 + 一行摘要回执。摘要文本由调用方按**事务内实际**恢复/降级的条目渲染（不按事前计划，计划可能与实际不符）。
 * @typedef {object} StoreTagInput - S5 治理：按 id 批量打标（coexist 面的 gap 标）。
 * @property {string[]} ids - 目标条目 id（1..MAX_CONSOLIDATE_MATCHES 个，须互不重复）。
 ...
 * @typedef {object} TidyRequestRow - tidy_requests 表 SELECT 行。
 * @property {string} id
 * @property {number} created_at
 * @property {string} status
 * @typedef {{id: string, createdAt: number, status: string}} TidyRequest - tidy_requests 表对外行（createdAt 驼峰）。
 ...
 * @property {(input: StoreSupersedeInput) => {superseded: MemoryEntry[], entry: MemoryEntry | null, cleared: TidyRequest | null}} supersedeEntries
 * @property {(input: StoreRollbackInput) => {restored: MemoryEntry[], demoted: MemoryEntry[]}} rollbackBatch
 * @property {(input: StoreIdListInput) => MemoryEntry[]} restoreEntries
 * @property {(input: StoreTagInput) => MemoryEntry[]} tagEntries
 ...
 * @property {() => TidyRequest | null} tidyRequestPending
 * @property {() => {request: TidyRequest, created: boolean}} tidyRequestAdd
 * @property {() => TidyRequest | null} tidyRequestClear
 * @property {(limit?: number) => TidyRequest[]} tidyRequestList
 * @property {() => void} close
 */
...
const ACTIVE_ONLY = `status = '${ACTIVE_STATUS}'`
...
/** 审计行 → 对象（列名映射的唯一出处：auditList 与按批查询共用同一形状）。 */
function toAudit(/** @type {AuditRow} */ row) {
  return {
    seq: row.seq,
    ts: row.ts,
    action: row.action,
...
/** 把 tidy_requests SELECT 行映射为稳定标记形状。 */
function toTidyRequest(/** @type {TidyRequestRow} */ row) {
  return {
    id: row.id,
    createdAt: row.created_at,
    status: row.status,
...
/**
 * 最早的 pending 标记行。正常只有一条（登记幂等 + 清除即结案）；这里仍按
 * created_at 排序取最早，避免历史 done 行或异常多行让「哪条是待整理」变得靠猜。
 * @param {Db} db - 连接。
 * @returns {TidyRequest | null} pending 行；没有则为 null。
 */
function pendingTidyRequest(/** @type {Db} */ db) {
  const row = /** @type {TidyRequestRow | undefined} */ (db.prepare("SELECT * FROM tidy_requests WHERE status = 'pending' ORDER BY created_at, id LIMIT 1").get())
  return row === undefined ? null : toTidyRequest(row)
}
...
export function openMemoryStore(dbPath, options = {}) {
  ...
  const store = {
     ...
     *
     * 可选把本批账目（`audit`）一并收进同一个事务：审计写在事务外时，写失败会留下
     * 「条目已降级、账本无该批次一行、batchReport 查无此批」的半成品，而半成品无迹可查——
     * 「既没写、又没人知道」正是要杜绝的那一格。
     * @param {StoreSupersedeInput} input - {ids, text?, tags?, facet?, level?, source?, workspaceKey?, agentKey?, sessionId?, batchId?, id?, audit?}。
     * @returns {{superseded: MemoryEntry[], entry: MemoryEntry | null, cleared: TidyRequest | null}} 被降级的旧条目、合并新条目（无 text 时 entry 为 null）与收边的待整理标记。
     */
    supersedeEntries(input) {
      ...
      return withTransaction(db, () => {
        // 桶内不跨在这里也把一道（纵深防御）：协议层有 assertSameBucket，但 Provider 是公开面，
        // 直调它跨桶合并会把 B 桶的正文静默搬进 A 桶。合并条目的桶继承 sources[0]，所以这条
        // 校验就是「产出条目落哪个桶」的前提。
        /** @type {MemoryEntry[]} */
        const superseded = []
        const firstTarget = getEntryOrNull(db, /** @type {string} */ (ids[0]))
        const first0 = firstTarget === null
          ? null
          : { track: firstTarget.track, scope: firstTarget.scope, agentKey: firstTarget.agentKey, workspaceKey: firstTarget.workspaceKey }
        for (const id of ids) {
          ...
          if (bucketKeyOf(entry) !== bucketKeyOf(/** @type {typeof entry} */ (first0))) {
            throw new InvalidInputError(`supersede must stay inside one bucket (track × scope × agentKey${entry.scope === 'workspace' ? ' × workspaceKey' : ''}): ${JSON.stringify(ids[0])} and ${JSON.stringify(id)} are not in the same bucket; nothing was changed`)
          }
          db.prepare('UPDATE entries SET status = ?, batch_id = ? WHERE id = ?').run(SUPERSEDED_STATUS, input.batchId ?? null, id)
        ...
        const entry = withEntry
          ? insertOne(db, {
              ...
              // 提前铸出的 id 必须一路传到插入口：中途掉一个环节，摘要行就只能退回事务外。
              ...(input.id === undefined ? {} : { id: input.id }),
            })
          : null
        // 待整理标记的收边也进同一笔：它是队列里的一条权威状态，留在事务外就等于
        // 「整批成了、标记没结」，下一轮重复消费一次。调用方给标记 id，Provider 认 id 改行。
        const cleared = input.audit !== undefined && input.audit.clearTidyRequestId !== undefined
          ? clearPendingTidyRequestById(db, input.audit.clearTidyRequestId)
          : null
        // 本批账目全部在同事务内编好并插入：batchId 与产出条目的 id 都是**落库前已知**的
        // （批次号由协议层铸、条目 id 由协议层提前给出），所以摘要行也能写在这里。账目与条目
        // 同一个提交，就不存在「条目已动、账本缺口」这一格，也就不需要事后补偿。
        if (input.audit !== undefined) {
          const { action, base, addText } = input.audit
          const ts = Date.now()
          for (const old of superseded) {
            // 降级行记的是**被降级条目自己的出处**，不是动作来源（动作来源落在产出行与摘要行上）。
            insertAuditRow(db, ts, { text: null, ...base, action, track: old.track, scope: old.scope, entryId: old.id, source: old.source })
          }
          if (entry !== null) {
            insertAuditRow(db, ts, { ...base, action: `${action}-add`, track: entry.track, scope: entry.scope, entryId: entry.id, text: entry.text, source: entry.source })
          }
          // 展开在前、显式字段在后：写成 `source: base.source, ...base` 的话，后者会把
          // 动作来源盖成别的（降级行那条曾把批次来源带偏），batchReport 的来源就会读错。
          insertAuditRow(db, ts, {
            ...base,
            action: 'consolidation',
            track: first.track,
            scope: first.scope,
            entryId: null,
            text: `superseded ${superseded.length} (${superseded.map((old) => old.id).join(', ')})${entry === null ? '; no merged entry' : `; merged entry ${entry.id} tagged ${addText}`}`,
            source: base.source,
          })
        }
        return { superseded, entry, cleared }
      })
    },

    /**
     * 事务内按 id 批量回滚（S5 §1）：降级过的条目置回 active（`superseded → active`），
     * 重新进入会话可见集。与 supersedeEntries 严格互逆：**只允许 superseded → active**，
     * 目标是 active（或未知 id）即响亮失败并整批回滚——回滚不是「改状态」的通用口子。
     * updated_at 刻意不动、version 不自增：条目没被改写，只是重新在场（时间线由审计行承担）。
     * @param {StoreIdListInput} input - {ids}。
     * @returns {MemoryEntry[]} 恢复后的条目（与 ids 同序）。
     */
    restoreEntries(input) {
      ...
      return withTransaction(db, () => {
        ...
        for (const id of ids) {
          const entry = getEntryOrNull(db, id)
          if (entry === null) throw new InvalidInputError(`no entry with id ${JSON.stringify(id)}; nothing was changed`)
          if (entry.status !== SUPERSEDED_STATUS) {
            throw new InvalidInputError(`entry ${JSON.stringify(id)} is ${entry.status}; only superseded entries can be restored`)
          }
          db.prepare('UPDATE entries SET status = ? WHERE id = ?').run(ACTIVE_STATUS, id)
          restored.push(/** @type {MemoryEntry} */ (getEntryOrNull(db, id)))
        }
        return restored
      })
    },
    ...
    /**
     * 事务内按 id 批量打标（S5 §2.3 coexist 面：两条都留，各打一枚 `gap` 标）。
     * 补进 tags（已有该标即幂等跳过，不重复写）；标签数越 `MAX_TAGS_PER_ENTRY` 时响亮失败，
     * 绝不静默丢弃。状态与 version 都不动——打标是元数据注记，不是改写。
     * @param {StoreTagInput} input - {ids, tag}。
     * @returns {MemoryEntry[]} 打标后的条目（与 ids 同序）。
     */
    tagEntries(input) {
      const ids = assertEntryIdList(input.ids, 'tag')
      if (typeof input.tag !== 'string' || input.tag.length === 0) {
        throw new InvalidInputError('entry tag must be a non-empty string')
      }
      const tag = assertStoredTags([input.tag])[0]
      return withTransaction(db, () => {
        /** @type {MemoryEntry[]} */
        const tagged = []
        for (const id of ids) {
          ...
          if (!entry.tags.includes(tag)) {
            ...
            if (tags.length > MAX_TAGS_PER_ENTRY) {
              throw new InvalidInputError(`entry ${JSON.stringify(id)} already has ${entry.tags.length} tags; adding ${JSON.stringify(tag)} would exceed ${MAX_TAGS_PER_ENTRY}`)
            }
            db.prepare('UPDATE entries SET tags = ? WHERE id = ?').run(JSON.stringify(tags), id)
          }
          tagged.push(/** @type {MemoryEntry} */ (getEntryOrNull(db, id)))
        }
        return tagged
      })
    },
     ...
     * @param {AuditInput} row - {action, track, scope, entryId, text, outcome, source, sessionId, batchId}。
     * @returns {object} 审计行（含 seq 与 ts）。
     */
    auditAppend(row) {
      const ts = Date.now()
      return { seq: insertAuditRow(db, ts, row), ts, ...row }
    },
    ...
    /**
     * 某动作的最近一行审计（只读；按 `seq` 倒序取第一条命中）。
     * 存在理由：靠审计流推断「上次某某是什么时候」**不能只看固定窗口**——审计行每天都在长，
     * 窗口一满，更早的记录就被挤出去，会被读成「从来没有过」。这里按动作精确取，与窗口无关。
     * @param {string} action - 动作名（如 'observed' / 'consolidation'）。
     * @returns {object | null} 最近一行；没有则 null。
     */
    lastAuditOf(action) {
      const row = db.prepare('SELECT * FROM audit WHERE action = ? ORDER BY seq DESC LIMIT 1').get(action)
      return row === undefined ? null : toAudit(/** @type {AuditRow} */ (row))
    },

    /**
     * 按批次号读审计行（纯只读：不落审计、不走审批门、不改任何行），按 seq 升序——
     * 「这一批干了什么」的重建面（降级行只记 id，产出条目另有 supersede-add 行）。
     * @param {string} batchId - 批次号（非空字符串）。
     * @returns {object[]} 该批次的审计行（升序；未知批次为空数组）。
     */
    auditByBatch(batchId) {
      assertBatchId(batchId)
      return /** @type {AuditRow[]} */ (db.prepare('SELECT * FROM audit WHERE batch_id = ? ORDER BY seq').all(batchId))
        .map(toAudit)
    },

    /**
     * 按批次号读条目（纯只读）：产出条目与降级条目都带该批次号（协议层的自动路径写入）。
     * @param {string} batchId - 批次号（非空字符串）。
     * @returns {MemoryEntry[]} 该批次涉及的条目（未知批次为空数组）。
     */
    entriesByBatch(batchId) {
      assertBatchId(batchId)
      return /** @type {EntryRow[]} */ (db.prepare('SELECT * FROM entries WHERE batch_id = ? ORDER BY created_at, id').all(batchId))
        .map(toEntry)
    },

    /**
     * 最近的批次号（纯只读）：按该批次最后一行审计时间倒序去重，可按来源过滤。
     * 面板留痕面用它列「最近几批自动整理」；不落审计、不走审批门、不改任何行。
     * @param {{source?: string, limit?: number}} [input] - {source?, limit?}（limit 缺省 5，最小 1）。
     * @returns {string[]} 批次号（新 → 旧）。
     */
    batchIds(input = {}) {
      const source = input.source
      if (source !== undefined && (typeof source !== 'string' || source.length === 0)) {
        throw new InvalidInputError('batch list source must be a non-empty string when given')
      }
      const limit = input.limit ?? 5
      if (!Number.isInteger(limit) || limit < 1) throw new InvalidInputError('batch list limit must be an integer >= 1')
      const sql = `SELECT batch_id, MAX(ts) AS latest FROM audit WHERE batch_id IS NOT NULL${source === undefined ? '' : ' AND source = ?'} GROUP BY batch_id ORDER BY latest DESC LIMIT ?`
      const rows = source === undefined
        ? /** @type {Array<{batch_id: string}>} */ (db.prepare(sql).all(limit))
        : /** @type {Array<{batch_id: string}>} */ (db.prepare(sql).all(source, limit))
      return rows.map((row) => row.batch_id)
    },

    /**
     * 按批次整批回滚（F8 批次面）：把该批次产出的合并条目降级、把该批次降级的源条目一次性
     * 恢复为 `active`——两件事在**同一个事务**里完成（要么全成、要么全不动）。只碰带该批次号的
     * 条目，非本批次条目零影响；校验全部前置，任一不满足即整批回滚、库零变更。
     * 已被手动救回的源条目不在恢复集合里（它已在场，无需再救）；整批已撤回由产物那一侧响亮拒绝。
     * @param {StoreRollbackInput} input - {batchId, producedIds}。
     * @returns {{restored: MemoryEntry[], demoted: MemoryEntry[]}} 恢复的源条目与降级的产出条目。
     */
    rollbackBatch(input) {
      assertBatchId(input.batchId)
      const producedIds = assertEntryIdList(input.producedIds, 'rollback product')
      const sourceIds = assertEntryIdList(input.sourceIds, 'rollback source')
      // 两张清单互斥（纵深防御）：同一条目既算产出又算源，会被先降级再恢复、且两张回执都报它。
      const overlap = producedIds.filter((id) => sourceIds.includes(id))
      if (overlap.length > 0) {
        throw new InvalidInputError(`rollback product and source id lists overlap (${overlap.join(', ')}); nothing was changed`)
      }
      return withTransaction(db, () => {
        /** @type {MemoryEntry[]} */
        const demoted = []
        for (const id of producedIds) {
          const entry = getEntryOrNull(db, id)
          if (entry === null) throw new InvalidInputError(`no entry with id ${JSON.stringify(id)}; nothing was changed`)
          if (entry.batchId !== input.batchId) {
            throw new InvalidInputError(`entry ${JSON.stringify(id)} belongs to batch ${JSON.stringify(entry.batchId)}, not ${JSON.stringify(input.batchId)}; nothing was changed`)
          }
          if (entry.status !== ACTIVE_STATUS) {
            throw new InvalidInputError(`entry ${JSON.stringify(id)} is already ${entry.status}; batch ${JSON.stringify(input.batchId)} looks already rolled back`)
          }
          demoted.push(entry)
        }
        // 源条目按**审计给的清单**逐条对账，而不是「查一遍当前批次号」：本批的源只能是
        // ①仍在本批名下的降级条目（待恢复）或 ②已被手工救回、仍在本批名下的在场条目（已在场，跳过）。
        // 被后一批重新吃掉（批次号换了）即响亮拒绝——否则撤回报成功、可见集却回不到整理前。
        /** @type {MemoryEntry[]} */
        const restored = []
        for (const id of sourceIds) {
          const entry = getEntryOrNull(db, id)
          if (entry === null) {
            throw new InvalidInputError(`source entry ${JSON.stringify(id)} is gone; batch ${JSON.stringify(input.batchId)} cannot be rolled back as a whole`)
          }
          if (entry.batchId !== input.batchId) {
            throw new InvalidInputError(`source entry ${JSON.stringify(id)} now belongs to batch ${JSON.stringify(entry.batchId)}; batch ${JSON.stringify(input.batchId)} can no longer be rolled back as a whole`)
          }
          if (entry.status === ACTIVE_STATUS) continue
          if (entry.status !== SUPERSEDED_STATUS) {
            throw new InvalidInputError(`source entry ${JSON.stringify(id)} is ${entry.status}; cannot roll back batch ${JSON.stringify(input.batchId)}`)
          }
          restored.push(entry)
        }
        for (const entry of demoted) {
          db.prepare('UPDATE entries SET status = ? WHERE id = ?').run(SUPERSEDED_STATUS, entry.id)
        }
        for (const entry of restored) {
          db.prepare('UPDATE entries SET status = ? WHERE id = ?').run(ACTIVE_STATUS, entry.id)
        }
        // 回执里的条目必须是**更新后**的快照：before 快照的 status 与库内真值相反
        // （被降级的报 active、被恢复的报 superseded），调用方拿到的是一份说谎的回执。
        // 用户可见文案此刻只用计数与 id，故无可见影响；但这正是「账与库一致」这条线
        // 的同类缺口，不能留。
        const demotedNow = demoted.map((entry) => /** @type {MemoryEntry} */ (getEntryOrNull(db, entry.id)))
        const restoredNow = restored.map((entry) => /** @type {MemoryEntry} */ (getEntryOrNull(db, entry.id)))
        // 撤回账目同事务：条目状态与账本要么一起成、要么一起不成。回执留在事务外时，
        // 一次审计写失败就能把账与库撕开——库已撤回而 rolledBack 仍为 false，面板按钮
        // 永远点不动（红队四轮 N2）。
        if (input.audit !== undefined) {
          const ts = Date.now()
          const { base, demoteOutcome, restoreOutcome } = input.audit
          for (const entry of demotedNow) {
            insertAuditRow(db, ts, { action: 'restore-batch-demote', track: entry.track, scope: entry.scope, entryId: entry.id, text: null, outcome: demoteOutcome, ...base })
          }
          for (const entry of restoredNow) {
            insertAuditRow(db, ts, { action: 'restore', track: entry.track, scope: entry.scope, entryId: entry.id, text: entry.text, outcome: restoreOutcome, ...base })
          }
          insertAuditRow(db, ts, {
            action: 'restore-batch',
            track: input.audit.track,
            scope: input.audit.scope,
            entryId: null,
            text: input.audit.summary({ restored: restoredNow, demoted: demotedNow }),
            outcome: restoreOutcome,
            source: base.source,
            ...base,
          })
        }
        return { restored: restoredNow, demoted: demotedNow }
      })
    },
    ...
    /**
     * 待整理的「全库整理」标记（规格 3.5.9 的排队式按钮）：用户点一下登记一条 pending，
     * 模型在会话内跑完 tidy 后由协议层清除。这是标记表，不是记忆：条目一条都不动。
     * @returns {TidyRequest | null} pending 行；没有则为 null。
     */
    tidyRequestPending() {
      return pendingTidyRequest(db)
    },

    /**
     * 登记一条「待整理」标记：只写标记，不调模型、不碰条目。
     * 幂等——已有一条 pending 就原样返回它，重复点击不堆行（created=true 仅首次）。
     * @returns {{request: TidyRequest, created: boolean}} 标记行与「本次是否新建」。
     */
    tidyRequestAdd() {
      const existing = pendingTidyRequest(db)
      if (existing !== null) return { request: existing, created: false }
      const id = randomUUID()
      const createdAt = Date.now()
      db.prepare("INSERT INTO tidy_requests (id, created_at, status) VALUES (?, ?, 'pending')").run(id, createdAt)
      return { request: { id, createdAt, status: 'pending' }, created: true }
    },

    /**
     * 清除待整理标记（模型跑完 tidy 后由协议层调用）：pending → done。行保留在库里，
     * 作为「用户点过、模型跑过」的痕迹（审计另有 tidy-request/cleared 一行）。
     * @returns {TidyRequest | null} 被清除的行（status='done'）；没有 pending 时为 null。
     */
    tidyRequestClear() {
      return clearPendingTidyRequest(db)
    },

    /**
     * 标记行清单（新→旧；面板与测试的观察面）。
     * @param {number} [limit] - 最多返回行数（缺省 20，硬上限 100）。
     * @returns {TidyRequest[]} 标记行。
     */
    tidyRequestList(limit = 20) {
      const capped = Number.isInteger(limit) && limit > 0 ? Math.min(limit, MAX_QUERY_LIMIT) : 20
      return /** @type {TidyRequestRow[]} */ (db.prepare('SELECT * FROM tidy_requests ORDER BY created_at DESC, id DESC LIMIT ?').all(capped))
        .map(toTidyRequest)
    },
  }
  return store
...
/**
 * id 列表校验（supersede / restore / tag 共用）：非空数组、≤ MAX_CONSOLIDATE_MATCHES、
 * 逐项非空字符串且互不重复（重复 id 会被第二遍当成「已降级/已在场」而误报，故入口就拒）。
...
/** 批次号入参防御：非空字符串（按批只读查询共用；形状非法响亮拒绝，不静默当空批）。 */
function assertBatchId(/** @type {unknown} */ batchId) {
  if (typeof batchId !== 'string' || batchId.length === 0) {
    throw new InvalidInputError('batch id must be a non-empty string')
  }
}
...
function insertOne(/** @type {Db} */ db, /** @type {StoreInsertInput} */ input) {
  ...
  // 调用方可提前给出 id（协议层为了让账目与条目同事务，必须先铸 id）。既然信了它，
  // 就得按同一条判据校验形状，并把唯一性冲突翻成结构化错误——畸形 id 静默落库、
  // 重复 id 抛裸 UNIQUE 错误，都是「失败要大声」这条红线的漏口。
  let id = randomUUID()
  if (input.id !== undefined) {
    if (typeof input.id !== 'string' || !ENTRY_ID_PATTERN.test(input.id)) {
      throw new InvalidInputError(`entry id must be a UUID v4 string (got ${JSON.stringify(input.id)})`)
    }
    // 归一成小写再落库：形状判据带 /i（接受大写写法），而 entries.id 是 SQLite 的 TEXT
    // PRIMARY KEY（二进制比较）——不归一，同一 UUID 的大写形式能再占一行，库内出现两个
    // lower(id) 相同的条目。归一之后「id 唯一」才是真的唯一。
    const lower = input.id.toLowerCase()
    id = /** @type {`${string}-${string}-${string}-${string}-${string}`} */ (lower)
  }
  const entry = {
    id,
    track,
  ...
  const INSERT_SQL = `INSERT INTO entries (id, track, scope, workspace_key, agent_key, text, source, tags, version, created_at, updated_at, session_id, facet, level, status, batch_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  try {
    db.prepare(INSERT_SQL)
      .run(entry.id, entry.track, entry.scope, entry.workspaceKey, entry.agentKey, entry.text, entry.source,
        JSON.stringify(tags), version, entry.createdAt, entry.updatedAt, entry.sessionId, facet, level, entry.status, entry.batchId)
  } catch (error) {
    // 唯一性冲突翻成结构化错误：调用方给的 id 与库里已有条目撞车时，裸 UNIQUE 报错
    // 既没说是哪个 id，也没带错误码，工具面读不出可行动的信息。
    if (/UNIQUE constraint failed: entries\.id/u.test(String(error?.message ?? ''))) {
      throw new InvalidInputError(`entry id ${JSON.stringify(entry.id)} already exists; nothing was changed`)
    }
    throw error
  }
  return entry
...
/**
 * 插入一条审计行（调用方给 ts，便于把一批审计钉在同一时刻）。Provider 内与事务内共用：
 * 事务路径（supersedeEntries）靠它把账本与条目改动收进同一笔，避免「条目落库、账本缺行」。
 * @param {Db} db
 * @param {number} ts - 审计时间戳（同一批用同一个值，便于把一批钉在同一时刻）。
 * @param {AuditInput} row
 * @returns {number} 该行的 seq（同一事务内可据此精确回填 entry_id，无需再查）。
 */
function insertAuditRow(/** @type {Db} */ db, ts, row) {
  const info = db.prepare(`INSERT INTO audit (ts, action, track, scope, entry_id, text, outcome, source, session_id, batch_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(ts, row.action, row.track ?? null, row.scope ?? null, row.entryId ?? null,
      row.text ?? null, row.outcome ?? null, row.source ?? null, row.sessionId ?? null, row.batchId ?? null)
  return Number(info.lastInsertRowid)
}

/**
 * 按 id 给待整理标记收边（pending → done），供事务内路径使用。行保留在库里，作为
 * 「用户点过、模型跑过」的痕迹。
 * @param {Db} db
 * @param {string} id - 标记行 id（写之前由调用方读出）。
 * @returns {TidyRequest | null} 被清除的行（status='done'）；未知 id 或不处于 pending 时为 null。
 */
function clearPendingTidyRequestById(/** @type {Db} */ db, id) {
  const row = /** @type {TidyRequestRow | undefined} */ (db.prepare('SELECT * FROM tidy_requests WHERE id = ?').get(id))
  if (row === undefined || row.status !== 'pending') return null
  db.prepare("UPDATE tidy_requests SET status = 'done' WHERE id = ?").run(id)
  return toTidyRequest({ ...row, status: 'done' })
}

/**
 * 给待整理标记收边（pending → done）。行保留在库里，作为「用户点过、模型跑过」的痕迹。
 * @param {Db} db
 * @returns {TidyRequest | null} 被清除的行；没有 pending 时为 null。
 */
function clearPendingTidyRequest(/** @type {Db} */ db) {
  const existing = pendingTidyRequest(db)
  if (existing === null) return null
  db.prepare("UPDATE tidy_requests SET status = 'done' WHERE id = ?").run(existing.id)
...
function migrate(/** @type {Db} */ db, /** @type {string} */ dbPath) {
  ...
  if (current < 7) {
    db.exec(TIDY_REQUESTS_SCHEMA_SQL)
    current = 7
  }
  if (current < 8) {
    applyAddColumns(db, V8_ADD_COLUMNS)
    db.exec(BATCH_INDEX_SQL)
    current = 8
  }
...
/**
 * v7 tidy_requests 表（面板「整理全库」按钮的排队标记，规格 3.5.9）：一行一次登记，
 * status 只有 pending → done 单向。表里没有正文、没有桶、没有会话归属——它只记
 * 「用户点过、还没被模型履约」，整理动作本身仍由模型在会话内显式跑（审计红线）。
 */
const TIDY_REQUESTS_SCHEMA_SQL = `
  CREATE TABLE IF NOT EXISTS tidy_requests (
    id         TEXT PRIMARY KEY,
    created_at INTEGER NOT NULL,
    status     TEXT NOT NULL CHECK (status IN ('pending', 'done'))
  );
  CREATE INDEX IF NOT EXISTS tidy_requests_status ON tidy_requests (status, created_at);
`

/**
 * v8 增量列：自动整理批次标识（F8 批次留痕）。`entries.batch_id` 记「本条目的产出／降级
 * 属于哪一批」，`audit.batch_id` 让审计能按批重建；两列皆可空（普通整理与手工写不产批次）。
 */
/** @type {Array<[string, string, string]>} */
const V8_ADD_COLUMNS = [
  ['entries', 'batch_id', 'TEXT'],
  ['audit', 'batch_id', 'TEXT'],
]

/** v8 批次索引：按批查询是留痕面（面板/命令面）的主路径，两表各建一支。 */
const BATCH_INDEX_SQL = `
  CREATE INDEX IF NOT EXISTS entries_batch_id ON entries (batch_id);
  CREATE INDEX IF NOT EXISTS audit_batch_id ON audit (batch_id);
`
