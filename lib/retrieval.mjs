 ...
 * @property {(query: string, entries: Array<{id: string, text: string, tags?: string[], recallCount?: number, lastRecalled?: number, updatedAt?: number}>, options?: {now?: number}) => Array<{id: string, text: string, tags?: string[], recallCount?: number, lastRecalled?: number, updatedAt?: number}>} retrieve
 *   检索：返回按加权分降序的全部命中（不截断；调用方自行 slice 并算 total/truncated）。
 *   可选第三参 `options.now` 只给「新旧加成」用（缺省 `Date.now()`）；第三方实现忽略它即可，契约不破。
 */
...
/**
 * 规格 3.3「层 B」零依赖半边：热度与新旧加成 ＋ 标签面折扣。全部为**可调默认**，
 * 并经 Config 的 `recall.weighting` 下发（设置页可改），故不再用模块级常量。
 * 三条数值的含义：
 * - `heat` / `heatSaturation` / `heatHalfLifeDays`：召回次数达到饱和度即吃满热度加成（乘性上限），
 *   再按「距上次召回多久」半衰期衰减——**热度要靠持续被召回来维持**，久未被召回即失温。
 * - `freshness` / `freshnessHalfLifeDays`：按半衰期衰减的新旧加成；**缺 `updatedAt` 取 0（乘子为 1）**——
 *   基线而非满额，否则「没时间戳」会反被当成最新，倒转旧口径。
 * - `tagDiscount`：词元只在 `tags` 命中时记的折扣（正文命中记满权 `1`）。
 */
/**
 * @typedef {object} RetrievalWeights - 检索加权表（每一键都可经 Config `recall.weighting` 覆盖）。
 * @property {number} heat - 热度加成上限（乘性；0 = 关闭）。
 * @property {number} heatSaturation - 召回次数达到此值即吃满热度加成。
 * @property {number} heatHalfLifeDays - 热度半衰期（天）；久未被召回即衰减。
 * @property {number} freshness - 新旧加成上限（乘性；0 = 关闭）。
 * @property {number} freshnessHalfLifeDays - 新旧半衰期（天）。
 * @property {number} tagDiscount - 词元只在 tags 命中时的折扣。
 */
...
/**
 * 合并一份加权参数：缺省即默认表（`RETRIEVAL_WEIGHTS`），传入的键覆盖同名项。
 * @param {Partial<RetrievalWeights> | undefined | null} overrides - 覆盖项。
 * @returns {RetrievalWeights} 合并后的加权表。
 */
function mergeWeights(overrides) {
  if (overrides === null || typeof overrides !== 'object') return RETRIEVAL_WEIGHTS
  return { ...RETRIEVAL_WEIGHTS, ...overrides }
}

/**
 * 热度因子（0..1）：**量**（召回次数 / 饱和度，封顶 1）× **衰减**（距上次召回的半衰期衰减）。
 *
 * 衰减是这一项存在的理由：只看累计次数的热度会形成「被召回 → 计数 +1 → 分更高 → 更常被召回」的
 * 增强回路，库一大就让少数条目垄断召回位。乘上衰减后，热度必须靠**持续被召回**来维持，
 * 久未被召回即失温——回路因此可退出（规格 3.3·丙）。缺次数或缺「上次召回」时间戳一律取 0。
 * @param {number | undefined} recallCount - 累计召回次数。
 * @param {number | undefined} lastRecalled - 上次召回时刻（epoch ms）。
 * @param {{now?: number, weights?: RetrievalWeights}} [options] - 基准时刻与加权表（缺省当前时刻与默认表）。
 * @returns {number} 0..1。
 */
export function heatFactorOf(recallCount, lastRecalled, options = {}) {
  const weights = options.weights ?? RETRIEVAL_WEIGHTS
  const now = options.now ?? Date.now()
  if (typeof recallCount !== 'number' || !Number.isFinite(recallCount) || recallCount <= 0) return 0
  if (typeof lastRecalled !== 'number' || !Number.isFinite(lastRecalled) || lastRecalled <= 0) return 0
  const volume = Math.min(1, recallCount / weights.heatSaturation)
  const ageDays = Math.max(0, (now - lastRecalled) / MS_PER_DAY)
  return volume * 2 ** (-ageDays / weights.heatHalfLifeDays)
}

/**
 * 新旧因子（0..1）：按半衰期衰减，**越新越大**；缺 `updatedAt`（或非正）取 **0**，
 * 即乘子为 1、不奖也不罚——**基线而非满额**。把「没时间戳」当成「最新」会把有时间戳的旧条目
 * 反而压下去，倒转 `rankOrder` 的旧口径（本仓测试就撞过这一处），故必须取基线。单调：更新的条目因子不会更小。
 * @param {number | undefined} updatedAt - 更新时刻（epoch ms）。
 * @param {{now?: number, weights?: RetrievalWeights}} [options] - 基准时刻与加权表（缺省当前时刻与默认表）。
 * @returns {number} 0..1。
 */
export function freshnessFactorOf(updatedAt, options = {}) {
  const weights = options.weights ?? RETRIEVAL_WEIGHTS
  const now = options.now ?? Date.now()
  if (typeof updatedAt !== 'number' || !Number.isFinite(updatedAt) || updatedAt <= 0) return 0
  const ageDays = Math.max(0, (now - updatedAt) / MS_PER_DAY)
  return 2 ** (-ageDays / weights.freshnessHalfLifeDays)
}
...
export class KeywordRetriever {
   ...
   * @param {Partial<RetrievalWeights>} [opts.weights] - 加权参数（缺省 `RETRIEVAL_WEIGHTS`；经 Config `recall.weighting` 下发）。
   */
  constructor(opts = {}) {
    this.id = 'keyword'
    this.name = 'Tokenized keyword'
    this.description = 'CJK-aware tokenized multi-term OR recall scored by coverage, token weight, heat, freshness and exact-phrase bonus (zero-dependency default path)'
    this.kind = /** @type {'substring'} */ ('substring')
    this.weights = mergeWeights(opts.weights)
  }

  /**
   * @param {string} query - 检索词（分词后任一词元命中即召回）。
   * @param {Array<{id: string, text: string, tags?: string[], recallCount?: number, lastRecalled?: number, updatedAt?: number}>} entries - 候选条目。
   * @param {{now?: number}} [options] - 可选：`now` = 热度与新旧加成的基准时刻（缺省 `Date.now()`）。
   * @returns {Array<{id: string, text: string, tags?: string[], recallCount?: number, lastRecalled?: number, updatedAt?: number}>} 命中条目（加权分降序，不截断）。
   */
  retrieve(query, entries, options) {
    const tokens = tokenize(query)
    if (tokens.length === 0) return [] // 空查询不返回全量（与 SubstringRetriever 一致）
    const raw = String(query).toLowerCase()
    const weights = this.weights
    const now = options !== null && typeof options === 'object' && typeof options.now === 'number' ? options.now : Date.now()
    const tokenLengthSum = tokens.reduce((sum, token) => sum + token.length, 0)
    /** @type {Array<{entry: {id: string, text: string, tags?: string[], recallCount?: number, lastRecalled?: number, updatedAt?: number}, relevance: number, score: number}>} */
    const hits = []
    for (const entry of entries) {
      const text = String(entry.text).toLowerCase()
      // 匹配面（规格 3.3·②）：正文记满权，只在 tags 命中记折权。tags 缺失为空时与旧行为逐字一致。
      const tagText = Array.isArray(entry.tags) ? entry.tags.join(' ').toLowerCase() : ''
      let hitWeightSum = 0
      let hitLengthWeightSum = 0
      for (const token of tokens) {
        const weight = text.includes(token) ? 1 : tagText.includes(token) ? weights.tagDiscount : 0
        if (weight === 0) continue
        hitWeightSum += weight
        hitLengthWeightSum += token.length * weight
      }
      if (hitWeightSum === 0) continue
      const coverage = hitWeightSum / tokens.length
      const weighted = hitLengthWeightSum / tokenLengthSum
      ...
      // 加权分（规格 3.3·③）：相关度 × 热度加成 × 新旧加成；缺失项取中性值，故无 tags/无热度时与旧排序一致。
      const score = relevance
        * (1 + weights.heat * heatFactorOf(entry.recallCount, entry.lastRecalled, { now, weights }))
        * (1 + weights.freshness * freshnessFactorOf(entry.updatedAt, { now, weights }))
      hits.push({ entry, relevance, score })
    }
    hits.sort((a, b) => b.score - a.score || b.relevance - a.relevance || rankOrder(a.entry, b.entry))
 ...
 *
 * **语义门槛（规格 3.3·层 B 收口）**：只有声明为语义的 provider（`semantic !== false`）才算可用。
 * 伪嵌入（哈希袋）不做语义建模，对本项目的中文语料几乎必零命中——放它进来等于用一个
 * 名为「语义召回」的开关**静默关掉召回**，故不计入可用性。
...
export function detectVectorBackend(opts = {}) {
  ...
  if (embedding === undefined || embedding === null) {
    return { available: false, sqliteVec: false, reason: 'no embedding provider available' }
  }
  if (embedding.semantic === false) {
    return { available: false, sqliteVec: false, reason: 'embedding provider is not semantic' }
  }
  return { available: true, sqliteVec: false }
}
