...
export const WARMUP_HEADER = {
  en: '[dsh-memento: warm-up block — the stable half of memory (speaking constraints + standing profile), frozen at session start. Workspace- and task-specific memory is NOT here: fetch it on demand with the memory_recall tool.]',
  zh: '[dsh-memento：预热块——记忆里稳定的一半（表达约束 ＋ 常驻画像），会话启动时冻结。工作区与任务相关的记忆不在此块：需要时用 memory_recall 工具按需取。]',
}
