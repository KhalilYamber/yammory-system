 ...
 * @property {string} restoreBatchNeedsId
 * @property {(batchId: string, restored: number, demoted: number, text: string, used: number, limit: number) => string} batchRolledBack
 * @property {(n: number, total?: number) => string} autoTidyPanelLine - n 为**本行展示**的条数；给了 total 且大于 n 时如实标注总数（截断不谎报）。
 * @property {(id: string, text: string) => string} autoTidyPanelSource
 * @property {(id: string, text: string) => string} autoTidyPanelProduced
 * @property {string} autoTidyPanelExpand
 * @property {string} autoTidyPanelCollapse
 * @property {string} autoTidyPanelRollback
 * @property {string} autoTidyPanelRolledBack
 * @property {(restored: number, demoted: number) => string} autoTidyPanelDone
...
export const COMMAND_TEXT = /** @type {{en: CommandTextBundle, zh: CommandTextBundle}} */ ({
  en: {
    usage: 'Usage: /memory list | query <word> | add <text> | remove <substring> | consolidate <substring...> => <new text> | restore <id...> | restore --batch=<id> | arbitrate <id...> | proposals [approve|dismiss <id>] | budgets | audit | adapters | export [--adapter=<id>] | import [--adapter=<id>] <path> | observe [--days=N] | session [on|off]',
    ...
    unknownVerb: (verb) => `Unknown subcommand "${verb}". Usage: /memory list | query <word> | add <text> | remove <substring> | consolidate <substring...> => <new text> | restore <id...> | restore --batch=<id> | arbitrate <id...> | proposals [approve|dismiss <id>] | budgets | audit | adapters | export [--adapter=<id>] | import [--adapter=<id>] <path> | observe [--days=N] | session [on|off]`,
    ...
    restoreBatchNeedsId: 'restore --batch needs a batch id: /memory restore --batch=<id> (the id comes from a tidy receipt or /memory audit)',
    batchRolledBack: (batchId, restored, demoted, text, used, limit) => `Rolled back batch ${batchId}: restored ${restored} source entr${restored === 1 ? 'y' : 'ies'} to active, demoted ${demoted} batch product${demoted === 1 ? '' : 's'} (kept on disk, out of every session\u2019s view):\n${text}\nLayer usage: ${used}/${limit}`,
    autoTidyPanelLine: (n, total) => `Auto-tidy batches in view: ${n}${total !== undefined && total > n ? ` (${total} in total)` : ''}`,
    autoTidyPanelSource: (id, text) => `source [${id}] ${text}`,
    autoTidyPanelProduced: (id, text) => `produced [${id}] ${text}`,
    autoTidyPanelExpand: 'Show details',
    autoTidyPanelCollapse: 'Hide details',
    autoTidyPanelRollback: 'Roll back this batch',
    autoTidyPanelRolledBack: 'Rolled back',
    autoTidyPanelDone: (restored, demoted) => `Rolled back: ${restored} restored to active, ${demoted} batch product demoted`,
  ...
  zh: {
    usage: '用法：/memory list | query <词> | add <文本> | remove <唯一子串> | consolidate <唯一子串...> => <新文本> | restore <id...> | restore --batch=<id> | arbitrate <id...> | proposals [approve|dismiss <id>] | budgets | audit | adapters | export [--adapter=<id>] | import [--adapter=<id>] <路径> | observe [--days=N] | session [on|off]',
    memoryEmpty: '记忆为空。',
    ...
    unknownVerb: (verb) => `未知子命令「${verb}」。用法：/memory list | query <词> | add <文本> | remove <唯一子串> | consolidate <唯一子串...> => <新文本> | restore <id...> | restore --batch=<id> | arbitrate <id...> | proposals [approve|dismiss <id>] | budgets | audit | adapters | export [--adapter=<id>] | import [--adapter=<id>] <路径> | observe [--days=N] | session [on|off]`,
    ...
    restoreBatchNeedsId: 'restore --batch 需要一个批次号：/memory restore --batch=<id>（批次号见整理回执或 /memory audit）',
    batchRolledBack: (batchId, restored, demoted, text, used, limit) => `已整批撤回 ${batchId}：恢复 ${restored} 条源条目为在场状态，降级 ${demoted} 条本批产出（留在库里，退出可见集）：\n${text}\n该层用量：${used}/${limit}`,
    autoTidyPanelLine: (n, total) => `本次自动整理 ${n} 组${total !== undefined && total > n ? `（共 ${total} 组）` : ''}`,
    autoTidyPanelSource: (id, text) => `源 [${id}] ${text}`,
    autoTidyPanelProduced: (id, text) => `产出 [${id}] ${text}`,
    autoTidyPanelExpand: '展开明细',
    autoTidyPanelCollapse: '收起明细',
    autoTidyPanelRollback: '撤回这一批',
    autoTidyPanelRolledBack: '已撤回',
    autoTidyPanelDone: (restored, demoted) => `已撤回：恢复 ${restored} 条为在场状态，降级 ${demoted} 条本批产出`,
 ...
 * 折成一个数字，让模型知道「这里还有东西可按需取」。报条数，另附 agent 轨（经验）
 * 的话题目录——只报话题名，正文一律不搬（话题来自 tags，不是条目正文）。
 */
export const WARMUP_DIRECTORY = {
  en: (/** @type {{onDemand: number, experienceCount: number, topics: string[]}} */ info) => {
    const base = `${info.onDemand} more workspace / agent-track entr${info.onDemand === 1 ? 'y stays' : 'ies stay'} out of this block`
    const directory = info.topics.length === 0 ? '' : ` (${info.experienceCount} experience entr${info.experienceCount === 1 ? 'y' : 'ies'}, topics: ${info.topics.join(', ')})`
    return `${base}${directory} — fetch them on demand with memory_recall.`
  },
  zh: (/** @type {{onDemand: number, experienceCount: number, topics: string[]}} */ info) => {
    const directory = info.topics.length === 0 ? '' : `（经验 ${info.experienceCount} 条，话题：${info.topics.join('、')}）`
    return `本工作区与 agent 轨另有 ${info.onDemand} 条记忆不在本块${directory}——需要时用 memory_recall 取。`
  },
}
