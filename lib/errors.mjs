...
/**
 * yammory_system 领域错误基类：稳定 code + 可公开的 details。
...
/**
 * 预算超限（v1 遗留错误）：v2 起写入永不因容量被拒，本错误不再抛出，仅保留码位供旧调用方识别。
...
/** 乐观锁冲突：审批期间目标条目已被并发写改动（version 前置条件不满足），响亮失败而非静默覆盖。 */
export class StaleWriteError extends MemoryError {
  /**
   * @param {{track: string, scope: string, match: string, expected: number, actual: number}} d - {track, scope, match, expected, actual}。
   */
  constructor(d) {
    super(
      ERROR_CODES.STALE_WRITE,
      `entry in ${d.track}/${d.scope} matching ${JSON.stringify(d.match)} changed while the write was being approved (expected version ${d.expected}, found ${d.actual}); re-read and retry`,
      d,
    )
...
export class DisabledError extends MemoryError {
  constructor() {
    super(ERROR_CODES.DISABLED, 'yammory_system is disabled by configuration; no memory service is available')
    this.name = 'DisabledError'
  }
}

/**
 * 会话级记忆开关关闭（F5）：该会话的写入/召回/观察一律拒绝。
 * 与审批门同级的前置拦截——在预算预检与 gate 之前抛出，任何调用路径都绕不过。
 */
export class SessionMemoryOffError extends MemoryError {
  /**
   * @param {string} [sessionId] - 被关闭的会话 id（审计与报错定位用）。
   * @param {string} [detail] - 附加说明（如被拒的具体动作）。
   */
  constructor(sessionId, detail) {
    const id = typeof sessionId === 'string' && sessionId.length > 0 ? sessionId : '(unknown)'
    super(
      ERROR_CODES.SESSION_MEMORY_OFF,
      `session ${id} has memory switched off (session-level switch)${detail === undefined ? '' : `; ${detail}`}; nothing was written or recalled`,
      { sessionId: id },
    )
    this.name = 'SessionMemoryOffError'
  }
}
...
/**
 * 观察扫描的读通道不可用：本 profile 没有 sessionQuery 服务。
 * 响亮降级（不抛异常打断回合，但结果里 ok:false ＋ 明确 code），绝不假装「没有历史」。
 */
export class SessionQueryUnavailableError extends MemoryError {
  constructor() {
    super(
      ERROR_CODES.SESSION_QUERY_UNAVAILABLE,
      'memory_observe scan needs the session-query service, which this profile does not provide; no history was read',
    )
    this.name = 'SessionQueryUnavailableError'
  }
}
