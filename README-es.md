...
| Surface | Kind | Notes |
|---|---|---|
| `memory` | tool | add/replace/remove/consolidate/supersede/query/tidy con guía Save/Skip; las entradas pueden llevar coordenadas de perfil (`facet` = una de las siete facetas, `level` = nivel de conocimiento por dominio); `supersede` fusiona 1..20 entradas en una con la etiqueta `merged` y degrada las antiguas a `superseded` (se conservan, nunca se borran), `tidy` devuelve un plan de solo lectura; las escrituras pasan por la puerta de aprobación |
