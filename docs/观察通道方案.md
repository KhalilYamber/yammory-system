...
> **S4b 实现状态（2026-09-13 回填）**：S4b-1..6 已落地（S4b-6 可选收尾项也已做），门链 8/8、`npm test` 263/263；交付明细与实测数字见 `施工清单.md` 的 S4b-0 / S4b 两行。**本文以下内容保持设计轮原文**（含「待验」标注与当时的行号），实现期的偏离与实测结论只在施工清单登记，不回改本文——设计轮与实现轮的口径分开留痕。
> **基线**：`HEAD = e2d525e`，测试 214/214，门链 8/8，工作区干净。
> **内核基线**：DSH checkout（本机 `<DSH checkout>`）；下文 `packages/...` 指该 checkout 内的路径，`~/.dsh/...` 指本机 DSH_HOME（`<DSH_HOME>`）。
...
| 会话日志形态 `session.v3.jsonl.zstd` | `~/.dsh/sessions/<workspace-key>/session-<id>…/` |
| 本项目会话目录 11 个 | 同上父目录 |
