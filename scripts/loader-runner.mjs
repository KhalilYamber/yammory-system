...
try {
  ...
  if (expected === 'reload') {
    ...
    const assertBase = (language) => {
      ...
      if (connection.list().length !== 7) throw new Error(`reload: expected 7 routes, got ${connection.list().length}`)
    }

    // Phase 1: initial mount — seam live, English description, 7 routes.
    assertBase('en')
