...
try {
  ...
  if (expected === 'reload') {
    ...
    const assertBase = (language) => {
      ...
      if (connection.list().length !== 6) throw new Error(`reload: expected 6 routes, got ${connection.list().length}`)
    }

    // Phase 1: initial mount — seam live, English description, 6 routes.
    assertBase('en')
