...
try {
  ...
  if (expected === 'reload') {
    ...
    const assertBase = (language) => {
      ...
      if (connection.list().length !== 5) throw new Error(`reload: expected 5 routes, got ${connection.list().length}`)
    }

    // Phase 1: initial mount — seam live, English description, 5 routes.
    assertBase('en')
