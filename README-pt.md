...
- **Observar, e deixar a observação correr sozinha.** O canal de observação lê um trecho limitado das suas próprias mensagens passadas e grava entradas de comportamento que o questionário não alcança; agora uma verificação somente leitura avisa quando a última observação tem mais de uma semana (uma linha de auditoria `observe-due` e uma linha no fim do próximo bloco de aquecimento), e uma ronda semanal agendada pode executá-la sem ninguém presente em UM espaço de trabalho: uma sessão `dsh` sem interface analisa, infere no máximo três entradas com evidência e as confirma pela mesma porta de aprovação, autorizada pela sua própria fonte de política de escrita (`source:observation`, `auto` / `ask` / `off`). Dentro do plugin não vive nenhum temporizador; o agendamento é uma entrada do Agendador de Tarefas do Windows, tal como a ronda de arrumação.
...
| `yammory-experience` | skill | Registra lições reutilizáveis do trabalho na trilha `agent` (fatos do ambiente, convenções, lições), separadas do perfil do usuário por uma única pergunta: esse conhecimento precisa estar presente em todo turno? Código-fonte: `skills/yammory-experience/` |
...
| web panel | client drawer | Somente leitura, dividido em dois mundos: a aba **memória** é uma árvore recolhível de sete facetas (categorias primeiro, entradas ao clicar) mais um bloco de nível de conhecimento; a aba **experiência** agrupa a trilha `agent` por tópico. Ambas compartilham busca, barras de orçamento, os três números observáveis e a cauda de auditoria; um botão de ação do usuário apenas enfileira uma arrumação da biblioteca inteira; a entrada lateral pode ser ocultada (`panel.enabled`) |
