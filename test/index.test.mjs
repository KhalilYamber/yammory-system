...
test('F1/F5/F6 语言面：language 控制工具描述与快照文案；apply 直调非法值响亮失败', (t) => {
  ...
  assert.ok(zhTool.description.includes('读写分层、带审批门'), 'zh 工具描述')
  assert.ok(zhTool.description.includes('应存（SAVE）'), 'zh Save/Skip 指引')
  const en = mount()
  t.after(() => teardown(en))
  const enTool = en.mock.tools.find((tool) => tool.name === 'memory')
  assert.ok(enTool.description.includes('Read and write the layered, approval-gated'), '默认 en 工具描述')
...
test('F6+S2：冻结预热段——会话内变更不更新注入；注入文本与 snapshot 审计逐字一致（可重建）', async (t) => {
  const mounted = mount({ writePolicy: 'auto' })
  t.after(() => teardown(mounted))
  const { mock } = mounted
  const service = mock.services.get('memory')
  ...
  assert.ok(first.includes('偏好中文回复'), '预热段包含已批准条目')
  assert.ok(first.includes('/2000 chars used'), '预热段带用量头')
...
test('S2：预热段只取 user-global；工作区层与 agent 轨不进预热，改走按需召回', async (t) => {
  const mounted = mount({ writePolicy: 'auto' })
  t.after(() => teardown(mounted))
  const { mock } = mounted
  const service = mock.services.get('memory')
  ...
  await service.add({ track: 'user', scope: 'workspace', text: '本工作区偏好' }, { agent: makeAgent(ws1) })
  await service.add({ track: 'agent', scope: 'user-global', text: '跨工作区环境事实' }, { agent: makeAgent(ws1) })
  await service.add({ track: 'user', scope: 'user-global', text: '全局偏好' }, { agent: makeAgent(ws1) })

  const textA = section.text({ agent: { session: ws1 } })
  assert.ok(textA.includes('全局偏好'), 'user-global 进预热段')
  assert.ok(!textA.includes('项目A约定'), 'agent/workspace 不进预热段（按需召回）')
  assert.ok(!textA.includes('本工作区偏好'), 'user/workspace 不进预热段（按需召回）')
  assert.ok(!textA.includes('跨工作区环境事实'), 'agent 轨不进预热段（按需召回）')

  const textB = section.text({ agent: { session: ws2 } })
  assert.ok(textB.includes('全局偏好'), 'user-global 跨工作区可见')

  // 按需那半照旧可达：工作区层条目仍在该工作区会话的 query 结果里
  const hits = service.query({ text: '项目A约定' }, { sessionId: 'ws1', workspaceKey: 'c:\\work\\proj-a' })
  assert.equal(hits.total, 1, '工作区条目仍可按需召回')
})
