...
import { PROTOCOL_URI, validateMemoryEntry, validateExportEnvelope, validateAuditRow, normalizeTags, normalizeFacet, normalizeLevel, normalizeStatus, MAX_TAGS_PER_ENTRY, MAX_TAG_LENGTH } from '../lib/protocol.mjs'
...
test('协议校验函数：facet/level/status 规范化与范围校验（v5 条目模型）', () => {
  // normalizeFacet / normalizeLevel / normalizeStatus
  assert.equal(normalizeFacet(undefined), null)
  assert.equal(normalizeFacet('心智'), '心智')
  assert.throws(() => normalizeFacet('不存在'), InvalidInputError)
  assert.equal(normalizeLevel(undefined), null)
  assert.equal(normalizeLevel(10), 10)
  assert.throws(() => normalizeLevel(0), InvalidInputError)
  assert.throws(() => normalizeLevel(11), InvalidInputError)
  assert.throws(() => normalizeLevel(3.5), InvalidInputError)
  assert.equal(normalizeStatus(undefined), 'active')
  assert.equal(normalizeStatus('superseded'), 'superseded')
  assert.throws(() => normalizeStatus('gone'), InvalidInputError)

  // validateMemoryEntry 接受带新字段的落盘条目，且缺省回填 null/null/active
  const valid = {
    id: '123e4567-e89b-42d3-a456-426614174000',
    track: 'user', scope: 'user-global', workspaceKey: '', agentKey: '',
    text: '带面与水平的条目', source: 'test', tags: [], version: 1,
    facet: '能力与技能', level: 7, status: 'active',
    createdAt: 1000, updatedAt: 2000, lastRecalled: null, recallCount: 0, sessionId: null,
  }
  const checked = validateMemoryEntry(valid)
  assert.equal(checked.facet, '能力与技能')
  assert.equal(checked.level, 7)
  assert.equal(checked.status, 'active')
  assert.equal(validateMemoryEntry({ ...valid, facet: undefined, level: undefined, status: undefined }).facet, null)
  assert.equal(validateMemoryEntry({ ...valid, facet: undefined, level: undefined, status: undefined }).level, null)
  assert.equal(validateMemoryEntry({ ...valid, facet: undefined, level: undefined, status: undefined }).status, 'active')
  assert.throws(() => validateMemoryEntry({ ...valid, facet: '不存在' }), InvalidInputError)
  assert.throws(() => validateMemoryEntry({ ...valid, level: 0 }), InvalidInputError)
  assert.throws(() => validateMemoryEntry({ ...valid, status: 'gone' }), InvalidInputError)
})
